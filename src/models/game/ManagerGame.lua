...
function ManagerGame.turnsLeft(self)
    return self._turnsLeft
end


function ManagerGame.setTurnsLeft(self, value)
    assert(self._turnsLeft >= 0)
    
    self._turnsLeft = value
    
    GameInfo:instance():managerStates():currentState():update(EControllerUpdate.ECUT_TURNS_COUNT)
    
    if(self._turnsLeft == 0)then
        self:onPlayerLose()
    end
end
...
function ManagerGame.tryChangeTo(self, catTo)
    ...
    if self:canMakeCombination(catFrom, catTo) or self:canMakeCombination(catTo, catFrom) then
        ...
        self:setTurnsLeft(self._turnsLeft - 1)
        
        if(not self._gameEnded)then
            self:pushes()
        end
